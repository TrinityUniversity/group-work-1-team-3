...
class GroupWork @Inject()(cc: ControllerComponents) extends AbstractController(cc) {
  ...
  def colorTemplate = Action {
    Ok(views.html.getFormTemplate())
  }

  def validateColor(username: String, color: String) = Action {
    Ok(s"username is : $username, Fav Color is: $color")
  }
